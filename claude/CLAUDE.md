...
- **NEVER** add decorative `echo "==="`-style headers to label command output for myself — that framing belongs in the reply text, and the extra segment just needs its own permission.
- **AVOID** using `echo "EXIT: $?"`-style pipe command, which forces a command to require its own permission.
